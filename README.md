...
- If you just want to identify potential transposases (not FULL or partial IS elements) in your sequences and don't like to install ISEScan, you can do so by following two steps: 1) download the transposase models (`clusters.faa.hmm` and `clusters.single.faa`) from ISEScan subdirectory [pHMMs](https://github.com/xiezhq/ISEScan/tree/master/pHMMs), 2) install and use software `HMMER` (version 3.1b2 or later) to search transposases in your sequences.
